...
const ProductoForm = ({ onProductoCreado, id: propsId }) => {
  // Hooks para navegación y parámetros de URL
  const navigate = useNavigate();
  const location = useLocation();
  const params = useParams();
  
  const isModal = !!onProductoCreado;
  
  // Determinamos si estamos realmente en la ruta de edición de productos
  const isEditingProductRoute = location.pathname.includes('/productos/editar/');
  
  // El ID viene de props o de la URL solo si estamos en la ruta de edición y NO es un modal
  const id = propsId || (!isModal && isEditingProductRoute ? params.id : null);
  ...
  return (
    <Container className={isModal ? "" : "mt-4"}>
      {!isModal && <h2 className="mb-4">{id ? 'Editar Producto' : 'Nuevo Producto'}</h2>}
