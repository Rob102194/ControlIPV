...
class SQLiteRecetaRepository(IRecetaRepository):
    ...
    def obtener_todos(self, sort_by: str = 'nombre', filter_by: str = None, grupo_id: str = None, producto_id: str = None) -> list[Receta]:
        """
        ...
        try:
            ...
            if grupo_id:
                query = query.filter(db_models.Receta.grupo_id == grupo_id)
            
            if producto_id:
                query = query.join(db_models.Ingrediente).filter(db_models.Ingrediente.producto_id == producto_id)
            ...
            for receta_db in recetas_db:
                ...
                receta = Receta(
                    ...
                    id=str(receta_db.id),
                    grupo_id=receta_db.grupo_id,
                    grupo_nombre=receta_db.grupo.nombre if receta_db.grupo else None
                )
                receta.ingredientes = [
                    Ingrediente(
    ...
    def actualizar(self, receta: Receta) -> Receta:
        ...
        try:
            ...
            receta_db.grupo_id = receta.grupo_id
