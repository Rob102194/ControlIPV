...
# Modelo para los grupos de recetas.
class Grupo(db.Model):
    __tablename__ = 'grupos'
    id = db.Column(db.String(36), primary_key=True, default=generate_uuid)
    nombre = db.Column(db.String(50), unique=True, nullable=False)
    recetas = db.relationship('Receta', back_populates='grupo')
...
class Receta(db.Model):
    ...
    grupo_id = db.Column(db.String(36), db.ForeignKey('grupos.id'), nullable=True)
    # Relaciones
    ingredientes = db.relationship('Ingrediente', back_populates='receta', cascade="all, delete-orphan")
    grupo = db.relationship('Grupo', back_populates='recetas')
